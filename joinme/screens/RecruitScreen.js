...
function RecruitScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (!profilesReady) {
      profiles.find({ userId: { $not: { $regex: `${currentUserId}`}}})
        .asArray()
        .then(res => {
  ...
  function _renderLightItem ({item, index}) {
    return <SliderEntry data={item} even={false} onEntryClick={handleEntryClick} onEntryDoubleClick={handleEntryDoubleClick} />;
  }
  ...
  const handleFollow = () => {
    ...
    if (!showDialog) { handleShowDialog() }
    followingListsCollection.findOne({userId: currentUserId})
      .then((res) => {
        if (res) {
          let fList = new Set(res.followingList);
          fList.add(selectedId);

          followingListsCollection.findOneAndUpdate(
            {userId: currentUserId}, 
            {$set: {followingList: Array.from(fList) }}
          )
            .then(res => {
              // Handle post-adding tasks
            })
        }
      })
  }
    ...
    <View style={[styles.exampleContainer, styles.exampleContainerDark ]}>
      ...
      {selectedUser && <View>
        <Dialog.Container visible={showDialog}>
          <Dialog.Title>New Following</Dialog.Title>
          <Dialog.Description>
            You are now following {selectedUser.firstName} {selectedUser.lastName}
          </Dialog.Description>
          <Dialog.Button label="OK" onPress={handleHideDialog}/>
      ...
      {filteredProfiles && 
        <Carousel
          ...
          useScrollView={true}
          ref={carousel} // May come in handy later
          onSnapToItem={(index) => { setSelectedUser(filteredProfiles[index]) }
          }
        />
      }
      <Modal 
